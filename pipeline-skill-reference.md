...
`content-create`은 LetsCoding 4인조 AI 에이전트 팀(🟡 Yellow, 🟣 Nana, 🟢 Green, 🔵 Blue)의 협업 파이프라인을 실행하는 Skill입니다.  
Yellow가 지휘자로서 사용자 요청을 분해하고 Kanban 태스크를 생성·모니터링·종합합니다.  
🔵 Blue는 `hyperframes` Skill을 통해 HTML 기반 동영상 렌더링도 담당합니다.
...
| `blue` | 🔵 Blue | gpt-5.5 | 디자이너·영상 (Designer / Video) | `~/Documents/AgentreeCompany/DesignTeam/` |

---

## 📐 파이프라인 패턴

### 0. 사전 조사 활용 (Skip Nana)

RND_Team/에 조사 문서가 이미 준비되어 있으면 Nana를 건너뛰고, 파일 경로를 직접 지정하여 Green/Blue에게 할당합니다.

```
사용자 요청
    │
    ├── RND_Team/ 사전 조사 파일 확인
    │
    ▼
🟢 Green (또는 🔵 Blue): 파일 경로 직접 입력
```

**사용 예시**:
```bash
# CSV 데이터 → Green 블로그 글
hermes kanban create "수출입 실적 분석 블로그 글" \
  --assignee green \
  --body "목표: 첨부 CSV 데이터를 분석하여 블로그 글 작성
입력: /home/leehg/Documents/AgentreeCompany/RND_Team/수출입 실적(품목별)_20260703.csv
완료 기준: BlogTeam/ 디렉토리에 YYYY-MM-DD_수출입-실적_post.md 저장" \
  --workspace dir:/home/leehg/Documents/AgentreeCompany/BlogTeam

# 사전 조사 .md → Blue 인포그래픽
hermes kanban create "주제: <주제> 인포그래픽 제작" \
  --assignee blue \
  --body "목표: RND_Team/ 사전 조사 파일 기반 인포그래픽
입력: /home/leehg/Documents/AgentreeCompany/RND_Team/<파일명>.md
완료 기준: DesignTeam/ 디렉토리에 PNG 저장" \
  --workspace dir:/home/leehg/Documents/AgentreeCompany/DesignTeam
```

> 💡 **Yellow 기준**: `RND_Team/`에 `.md`, `.csv`, `.json` 파일이 있으면 내용을 읽고, 적합한 워커에게 바로 할당. Nana 태스크를 불필요하게 만들지 않는다.
...
### 4. 동영상 콘텐츠 (Nana → Green → Blue [hyperframes])
...
🟣 Nana: 조사 (research)
    │
    ▼
🟢 Green: 스크립트 작성 (script)
    │
    ▼
🔵 Blue: hyperframes 동영상 렌더링 (video)
    │
    ▼
🟡 Yellow: 결과 종합
```
...
```bash
# 1단계: Nana 조사
hermes kanban create "주제: <주제> 조사" \
  --assignee nana \
  --body "목표: ... 입력: 없음 완료 기준: RND_Team/에 마크다운 저장" \
  --workspace dir:/home/leehg/Documents/AgentreeCompany/RND_Team

# 2단계: Green 스크립트 작성
hermes kanban create "주제: <주제> 영상 스크립트" \
  --assignee green \
  --parent <nana_task_id> \
  --body "목표: Nana 조사 기반 영상 스크립트 + 나레이션 작성
입력: 부모 태스크 결과
완료 기준: BlogTeam/에 스크립트 저장, 타임라인·비주얼 방향 포함" \
  --workspace dir:/home/leehg/Documents/AgentreeCompany/BlogTeam

# 3단계: Blue hyperframes 동영상 렌더링
hermes kanban create "주제: <주제> 동영상 렌더링" \
  --assignee blue \
  --parent <green_task_id> \
  --body "목표: Green 스크립트 기반 hyperframes 동영상 제작
입력: 부모 태스크 결과 (스크립트)
규격: 1080x1920 (숏폼) 또는 1920x1080 (유튜브)
완료 기준: DesignTeam/에 MP4 저장, ffprobe 검증 완료" \
  --workspace dir:/home/leehg/Documents/AgentreeCompany/DesignTeam
```
...
| 🔵 Blue (영상) | `DesignTeam/` | `YYYY-MM-DD_<주제>_video.mp4` | `2026-07-03_ai-trends_video.mp4` |
...
- Skill 위치: `~/.hermes/profiles/yellow/skills/devops/content-create/SKILL.md`
- Hyperframes Skill: `~/.hermes/profiles/yellow/skills/creative/hyperframes/SKILL.md`
